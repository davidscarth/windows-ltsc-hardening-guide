...
3. **Compare:** The hash generated by the command must exactly match the official hash from Microsoft (as provided in this guide). If it does not, DO NOT USE THE FILE. Instead, re-download a known good copy.
...
3.  **Click START**. A set of customization options will appear, uncheck all and click 'OK'. A warning will appear that all data on the USB drive will be destroyed. Click `OK` to continue.
...
3.  **Apply Microsoft Security Baseline:**
